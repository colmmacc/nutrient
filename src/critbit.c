#define _POSIX_C_SOURCE 200112

#include <stdint.h>
#include <string.h>
#include <stdlib.h>

#include <sys/types.h>
#include <errno.h>

#include "critbit.h"

typedef struct
{
    void *child[2];
    uint32_t byte;
    uint8_t otherbits;
} critbit0_node;

static void *_critbit0_find(critbit0_tree * t, const char *key,
                            uint32_t key_len)
{
    uint8_t *p = t->root;
    uint32_t found_key_len;
    uint32_t found_value_len;

    if (!p)
        return NULL;

    while (1 & (intptr_t) p) {
        critbit0_node *q = (void *) (p - 1);
        uint8_t c = 0;
        if (q->byte < key_len)
            c = key[q->byte];
        const int direction = (1 + (q->otherbits | c)) >> 8;

        p = q->child[direction];
    }

    memcpy(&found_key_len, p, sizeof(uint32_t));
    memcpy(&found_value_len, p + sizeof(uint32_t), sizeof(uint32_t));

    if (found_key_len == key_len
        && 0 == memcmp(key,
                       ((const char *) p) + sizeof(uint32_t) + sizeof(uint32_t),
                       key_len)) {
        return p;
    }

    return NULL;
}

static int critbit0_allocator(void **memory, size_t size)
{
    return posix_memalign(memory, sizeof(void *), size);
}

static int (*allocator) (void **, size_t) = critbit0_allocator;

void critbit0_set_allocator(int (*func) (void **, size_t))
{
    allocator = func;
}

static void critbit0_deallocator(void *memory, size_t size)
{
    free(memory);
}

static void (*deallocator) (void *, size_t size) = critbit0_deallocator;

void critbit0_set_deallocator(void (*func) (void *, size_t))
{
    deallocator = func;
}

int critbit0_find(critbit0_tree * t, const char *key, uint32_t key_len,
                  const char **value, uint32_t * value_len)
{
    void *r = _critbit0_find(t, key, key_len);;

    if (!r)
        return -1;

    /* Copy the value length */
    memcpy(value_len, (const char *) r + sizeof(uint32_t), sizeof(uint32_t));

    /* Point the value to the right data */
    *value = (const char *) r + sizeof(uint32_t) + sizeof(uint32_t) + key_len;

    return 0;
}

int critbit0_insert(critbit0_tree * t, const char *key, uint32_t key_len,
                    const char *value, uint32_t value_len)
{
    const uint8_t *const ubytes = (void *) key;
    uint8_t *p = t->root;

    if (!p) {
        char *x;
        int a =
            allocator((void **) &x,
                      sizeof(key_len) + key_len + sizeof(value_len) + value_len);
        if (a)
            return 0;

        /* Copy the key and value length */
        memcpy(x, &key_len, sizeof(uint32_t));
        memcpy(x + sizeof(uint32_t), &value_len, sizeof(uint32_t));

        /* Copy the key */
        memcpy(x + sizeof(uint32_t) + sizeof(uint32_t), key, key_len);

        /* Copy the value */
        memcpy(x + sizeof(uint32_t) + sizeof(uint32_t) + key_len, value, value_len);

        t->root = x;
        return 2;
    }

    while (1 & (intptr_t) p) {
        critbit0_node *q = (void *) (p - 1);
        uint8_t c = 0;
        if (q->byte < key_len)
            c = ubytes[q->byte];
        const int direction = (1 + (q->otherbits | c)) >> 8;

        p = q->child[direction];
    }

    uint32_t newbyte;
    uint32_t newotherbits;

    for (newbyte = 0; newbyte < key_len; ++newbyte) {
        if (p[sizeof(uint32_t) + sizeof(uint32_t) + newbyte] != ubytes[newbyte]) {
            newotherbits =
                p[sizeof(uint32_t) + sizeof(uint32_t) + newbyte] ^ ubytes[newbyte];
            goto different_byte_found;
        }
    }

    if (p[newbyte] != 0) {
        newotherbits = p[sizeof(uint32_t) + sizeof(uint32_t) + newbyte];
        goto different_byte_found;
    }
    return 1;

  different_byte_found:

    while (newotherbits & (newotherbits - 1))
        newotherbits &= newotherbits - 1;
    newotherbits ^= 255;
    uint8_t c = p[sizeof(uint32_t) + sizeof(uint32_t) + newbyte];
    int newdirection = (1 + (newotherbits | c)) >> 8;

    critbit0_node *newnode;
    if (allocator((void **) &newnode, sizeof(critbit0_node)))
        return 0;

    char *x;
    if (allocator((void **) &x, sizeof(uint32_t) + sizeof(uint32_t) + key_len)) {
        deallocator(newnode, sizeof(critbit0_node));
        return 0;
    }

    memcpy(x, &key_len, sizeof(uint32_t));
    memcpy(x + sizeof(uint32_t), &value_len, sizeof(uint32_t));
    memcpy(x + sizeof(uint32_t) + sizeof(uint32_t), key, key_len);
    memcpy(x + sizeof(uint32_t) + sizeof(uint32_t) + key_len, value, value_len);

    newnode->byte = newbyte;
    newnode->otherbits = newotherbits;
    newnode->child[1 - newdirection] = x;

    void **wherep = &t->root;
    for (;;) {
        uint8_t *p = *wherep;
        if (!(1 & (intptr_t) p))
            break;
        critbit0_node *q = (void *) (p - 1);
        if (q->byte > newbyte)
            break;
        if (q->byte == newbyte && q->otherbits > newotherbits)
            break;
        uint8_t c = 0;
        if (q->byte < key_len)
            c = ubytes[q->byte];
        const int direction = (1 + (q->otherbits | c)) >> 8;
        wherep = q->child + direction;
    }

    newnode->child[newdirection] = *wherep;
    *wherep = (void *) (1 + (char *) newnode);

    return 2;
}

int critbit0_delete(critbit0_tree * t, const char *key, uint32_t key_len)
{
    const uint8_t *ubytes = (void *) key;
    uint8_t *p = t->root;
    void **wherep = &t->root;
    void **whereq = 0;
    critbit0_node *q = 0;
    int direction = 0;
    uint32_t found_key_len;
    uint32_t found_value_len;

    if (!p)
        return 0;

    while (1 & (intptr_t) p) {
        whereq = wherep;
        q = (void *) (p - 1);
        uint8_t c = 0;
        if (q->byte < key_len)
            c = ubytes[q->byte];
        direction = (1 + (q->otherbits | c)) >> 8;
        wherep = q->child + direction;
        p = *wherep;
    }

    memcpy(&found_key_len, p, sizeof(uint32_t));
    memcpy(&found_value_len, p + sizeof(uint32_t), sizeof(uint32_t));

    if (found_key_len != key_len
        || 0 != memcmp(key,
                       ((const char *) p) + sizeof(uint32_t) +
                       sizeof(uint32_t), key_len))
        return 0;

    deallocator(p, sizeof(uint32_t) + sizeof(uint32_t) + key_len + found_value_len);

    if (!whereq) {
        t->root = 0;
        return 1;
    }

    *whereq = q->child[1 - direction];
    deallocator(q, sizeof(critbit0_node));

    return 1;
}

static void traverse(void *top)
{
    uint8_t *p = top;

    if (1 & (intptr_t) p) {
        critbit0_node *q = (void *) (p - 1);
        traverse(q->child[0]);
        traverse(q->child[1]);
        deallocator(q, sizeof(critbit0_node));
    }
    else {
        deallocator(p, sizeof(critbit0_node));
    }
}

void critbit0_clear(critbit0_tree * t)
{
    if (t->root)
        traverse(t->root);
    t->root = NULL;
}

static int
allprefixed_traverse(uint8_t * top,
                     int (*handle) (const char *, uint32_t, const char *, uint32_t, void *),
                     void *arg)
{
    uint32_t key_len;
    uint32_t value_len;

    if (1 & (intptr_t) top) {
        critbit0_node *q = (void *) (top - 1);
        for (int direction = 0; direction < 2; ++direction)
            switch (allprefixed_traverse(q->child[direction], handle, arg)) {
            case 1:
                break;
            case 0:
                return 0;
            default:
                return -1;
            }
        return 1;
    }

    memcpy(&key_len, top, sizeof(uint32_t));
    memcpy(&value_len, top + sizeof(uint32_t), sizeof(uint32_t));

    return handle((const char *) top + sizeof(uint32_t) + sizeof(uint32_t),
                  key_len, (const char *) top + sizeof(uint32_t) + sizeof(uint32_t) + key_len, value_len, arg);
}

int
critbit0_allprefixed(critbit0_tree * t, const char *prefix, uint32_t prefix_len,
                     int (*handle) (const char *, uint32_t, const char *, uint32_t, void *),
                     void *arg)
{
    const uint8_t *ubytes = (void *) prefix;
    uint8_t *p = t->root;
    uint8_t *top = p;

    if (!p)
        return 1;

    while (1 & (intptr_t) p) {
        critbit0_node *q = (void *) (p - 1);
        uint8_t c = 0;
        if (q->byte < prefix_len)
            c = ubytes[q->byte];
        const int direction = (1 + (q->otherbits | c)) >> 8;
        p = q->child[direction];
        if (q->byte < prefix_len)
            top = p;
    }

    for (size_t i = 0; i < prefix_len; ++i) {
        if (p[i] != ubytes[i])
            return 1;
    }

    return allprefixed_traverse(top, handle, arg);
}
